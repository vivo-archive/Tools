...
public class FTPTextFileRecordHandler extends RecordHandler {
	 ...
	 * 
	 */
	public FTPTextFileRecordHandler(String hostAddr, String hostPort, String connUN, String connPW, String fileDir) throws NumberFormatException, SocketException, IOException {
	...
	private class FTPTextFileRecordIterator implements Iterator<Record> {
		...
		protected FTPTextFileRecordIterator() {
			...
			try {
				for(FTPFile filename : FTPTextFileRecordHandler.this.ftp.listFiles()) {
					if(filename.isFile()) {
